...
Children are rendered on the first paint too — wrapped in `AblyReadyContext.Provider value={false}` — so a descendant can call `useAblyReady()` and render a placeholder while the client is being created. Just don't call an `ably/react` hook unconditionally before checking `useAblyReady()`, since `AblyProvider` isn't above the tree yet on that initial render.
