...
describe('instant validation', () => {
  ...
  describe.each(cases)('$description', ({ isClientNav }) => {
    ...
    async function navigateTo(href: string) {
      ...
      const indexPage = // (force newline for nicer alignment)
        href.startsWith('/shells/')
          ? href.startsWith('/shells/with-root-param/')
            ? '/shells/with-root-param'
            : '/shells'
          : href.startsWith('/default/')
            ? '/default'
            : '/suspense-in-root'
